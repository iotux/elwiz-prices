const { skewDays } = require("../utils/util");
...
function parseJsonSafely(message, logger = console) {
  ...
  try {
    ...
    if (message === null || typeof message.toString !== "function") {
      if (logger && logger.error) {
        logger.error(
          "[parseJsonSafely] Received null message or message without toString method.",
        );
      }
      return {
        error: true,
        message: "Invalid message object received (null or no toString).",
        data: null,
      };
    }
    ...
    if (logger && logger.error) {
      logger.error(
        `[parseJsonSafely] Error converting message to string: ${e.message}`,
      );
    }
    return {
      error: true,
      message: `Error converting message to string: ${e.message}`,
      data: null,
    };
  }
  ...
  if (typeof messageString !== "string") {
    if (logger && logger.error) {
      logger.error(
        `[parseJsonSafely] message.toString() did not return a string. Type was: ${typeof messageString}`,
      );
    }
    return {
      error: true,
      message: `message.toString() did not return a string (type: ${typeof messageString}).`,
      data: null,
    };
  }
  ...
  if (trimmedString === "") {
    if (logger && logger.info) {
      ...
      logger.info(
        `[parseJsonSafely] Received empty string after trim. Original (if string): "${messageString.length > 100 ? messageString.substring(0, 100) + "..." : messageString}"`,
      );
    }
    return {
      error: true,
      message: "Empty string cannot be parsed as JSON.",
      data: null,
    };
  }
  ...
  try {
    ...
    if (logger && logger.error) {
      logger.error(
        `[parseJsonSafely] Error parsing JSON: ${e.message} for input (first 100 chars): "${trimmedString.substring(0, 100)}"`,
      );
    }
    ...
    return {
      error: true,
      message: `Error parsing JSON: ${errorMessage}`,
      data: null,
    };
  }
}
...
class PriceService {
    ...
    this.mqttClient.on("message", (msgTopic, message) => {
      if (msgTopic.startsWith(this.priceTopic)) {
        if (message.length === 0) {
          if (this.debug)
            this.logger.info(
              `[PriceService] Received empty message on ${msgTopic}. Ignoring.`,
            );
          return;
        }
        if (this.debug)
        ...
        if (
          ...
          this.logger.error(
            `[PriceService] Error processing price message for topic ${msgTopic}. Details: ${detailMessage}`,
          );
        }
      }
    });
  ...
  processReceivedPrices() {
    ...
    this.twoDaysData.forEach((data) => {
      if (!data || !data.priceDate) {
        if (this.debug)
          this.logger.warn(
            "[PriceService] Skipping invalid data entry in twoDaysData.",
          );
        return;
      }
      ...
      if (dataDate === todayStr) {
        ...
        if (this.debug)
          this.logger.info(
            `[PriceService] Assigned data for ${dataDate} to dayPrices.`,
          );
        ...
        if (this.debug)
          this.logger.info(
            `[PriceService] Assigned data for ${dataDate} to nextDayPrices.`,
          );
        ...
        if (this.debug)
          this.logger.info(
            `[PriceService] Assigned data for ${dataDate} to prevDayPrices.`,
          );
      } else {
        ...
        if (this.debug)
          this.logger.warn(
            `[PriceService] Data for ${dataDate} in twoDaysData (${this.twoDaysData.map((d) => d.priceDate).join(", ")}) does not match today (${todayStr}), tomorrow (${tomorrowStr}), or yesterday (${yesterdayStr}).`,
          );
      }
    });
    ...
    if (!this.dayPrices.priceDate) {
      if (this.debug) {
        this.logger.warn(
          `[PriceService] Today's (${todayStr}) price data is not available in twoDaysData. dayPrices will be empty. prevDayPrices is ${this.prevDayPrices.priceDate || "empty"}. nextDayPrices is ${this.nextDayPrices.priceDate || "empty"}`,
        );
      }
    }
  ...
  performMidnightRollover() {
    ...
    if (this.debug) {
      this.logger.info(
        `[PriceService] Midnight Rollover: Running for new date ${newTodayStr}. Triggering price processing.`,
      );
    }
    ...
    if (this.debug) {
      this.logger.info(
        `[PriceService] Midnight Rollover: Price processing complete for ${newTodayStr}.`,
      );
      this.logger.info(
        `  New dayPrices: ${this.dayPrices.priceDate || "None"}`,
      );
      this.logger.info(
        `  New nextDayPrices: ${this.nextDayPrices.priceDate || "None"}, Available: ${this.nextDayAvailable}`,
      );
      this.logger.info(
        `  New prevDayPrices: ${this.prevDayPrices.priceDate || "None"}`,
      );
    }
  }
